...
import { getReviewTasks, submitReviewedTask } from '@/services/manager';
...
export const ReviewTasks = () => {
  ...
  const confirmSubmit = (task: UiTask) => {
    Alert.alert(
      'Confirm Submission',
      `Are you sure you want to ${task.status === 'completed'? "approve" : "reject"} "${task.title}" task?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              await submitReviewedTask({ id: task.id, status: task.status });

              Alert.alert('Success', 'Task reviewed successfully');
        ...
        <ManagerLayout>
          <View className="flex-1 justify-center items-center">
            <Text className="text-gray-500 text-lg text-center">
              Wohoo! No tasks for review today 😊
            </Text>
          </View>
        </ManagerLayout>
      );
